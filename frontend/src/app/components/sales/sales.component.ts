...
export class SalesComponent implements OnInit {
  ...
  // Reports
  exportAsXLSX(): void {
    this.saleService.exportAsExcelFile(this.saleService.sales, 'Ventas');
  }
  ...
  downloadSalePDF() {
    ...
    doc.save('Ventas-' + new Date().getTime() + '.pdf')
  }
  ...
  generateHeader(doc) {
    ...
    doc.text("Ventas - SAM 13", 50, 22, { align: "center" });
  }
  generateInformation(doc) {
    var id = document.getElementById('table-sales');
    doc.autoTable(
      {
        margin: {
          bottom: 60,
